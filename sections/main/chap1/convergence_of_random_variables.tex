...
\begin{definition}[Weak Convergence of Measures]
    Let $\{\mu_n\}_{n=1}^\infty$ be a sequence of measures on a measurable space $(X, \mathcal{F})$. We say that $\mu_n$ converges weakly to a measure $\mu$ if:
    \begin{equation}
        \int f d\mu_n \to \int fd\mu,
    \end{equation} 
    \noindent for all $f$ continuous and bounded.
\end{definition} 

\begin{definition}[Continuity Sets] 
    A continuity set $A\in\mathcal{B}$, where $\mathcal{B}$ is the Borel-$\sigma$-algebra, with respect to a measure $\mu$ satisfies:
    \begin{equation}
        \mu(\partial A) = 0, 
    \end{equation} 
