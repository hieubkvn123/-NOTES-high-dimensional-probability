...
\begin{proof*}[Proposition \ref{prop:alt_rep_of_fisher}]
    ...
    \begin{align*}
        \E_{\btheta}[s(\btheta;X)] &= \int_\chi p_{\btheta}(x) \frac{\partial\log p_\xi(x)}{\partial\xi}\Bigg|_{\xi=\btheta}dx \\
            ...
            &= \frac{\partial}{\partial\btheta}\int_\chi p_{\btheta}(x)dx = \frac{\partial}{\partial\btheta}1 = 0.
    \end{align*} 

    \noindent Therefore, we have $\boxed{\mathcal{I}_{\bX}(\btheta) = \mathrm{Var}(s(\btheta;\bX))}$. $\color{red}\dots\color{black}$
\end{proof*} 

\begin{remark}[Exchanging Derivative and Integral]
    Note that in the proof of proposition \ref{prop:alt_rep_of_fisher}, we exchanged the integration and the partial derivative to prove that $\E_{\btheta}[s(\btheta;X)]=0$. $\color{red}\dots\color{black}$
\end{remark} 
