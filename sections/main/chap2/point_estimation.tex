...
\begin{proof*}[Proposition \ref{prop:consistency_of_mle}]
    ...
    \noindent Then, by $\ULLN$, we have $\|M_n - M\|_\infty \xrightarrow{p} 0$. This means that for any fixed $\epsilon>0$, there exists $\delta\in(0,1)$ and $N\in\mathbb{N}$ such that for all $n\ge N$, with probability of at least $1-\delta$, we have:
    \begin{align*}
