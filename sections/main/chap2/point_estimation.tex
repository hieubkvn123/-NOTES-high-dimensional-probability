...
\begin{proposition}{Asymptotic Normality of MLE}{asymptotic_normality_mle}
    ...
    \begin{equation}
        \sqrt{n}(\Theta_{MLE} - \btheta) \xrightarrow{d} \mathcal{N}(0, \mathcal{I}(\btheta)^{-1}).
    \end{equation}
\end{proposition} 
...
\begin{definition}[Fisher Information]
    Let $\bX=(X_1, \dots, X_n) \sim p_{\btheta}$ be a random sample from a distribution parameterized by $\btheta$. The \underline{(total) Fisher Information} about $\theta$ in the random sample $\bX$ is defined as follows:
    \begin{equation}
        \mathcal{I}_\bX(\theta) = \E_{\theta}\biggSquare{
            \biggRound{
                \frac{\partial}{\partial\xi}\log L(\xi;\bX)
            }^2
        },
    \end{equation}

    \noindent where $\E_{\theta}$ denotes the expectation taken over the distribution $p_{\theta}$ parameterized by $\theta$. The Fisher Informaton is the total information about $\theta$ contained in the sample $\bX$.
\end{definition}

\begin{proposition}{Alternative Representation of Fisher Information}{alt_rep_of_fisher}
    Let $\bX=(X_1, \dots, X_n) \sim p_{\btheta}$ be a random sample from a distribution parameterized by $\btheta$. Let $\ell(\theta;\bX)=\log L(\theta;\bX)$ be the log-likelihood function. Then, we have:
    \begin{equation}
        \mathcal{I}_{\bX}(\theta) = 
    \end{equation} 
\end{proposition} 
