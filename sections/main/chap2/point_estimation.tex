...
\begin{proof*}[Proposition \ref{prop:consistency_of_mle}]
    ...
    \noindent Then, by $\ULLN$, we have $\|M_n - M\|_\infty \xrightarrow{p} 0$. Therefore, since $\Theta$ is compact, we also have:
    \begin{align*}
        \arg\max_{\theta\in\Theta}M_n(\theta) \xrightarrow{p} \arg\max_{\theta\in\Theta}M(\theta).
    \end{align*} 

    \noindent In other words, $\Theta_{MLE}\xrightarrow{p}\btheta$.
\end{proof*} 
